...
Shader "Custom/Sphere Warp" 
{
	...
	SubShader 
	{
		...
		Pass 
		{
			...
			{
				float x = InnerSphereRadius * pow( 1 + ( Pi / _BlockStageWidth), r );
				return x / pow( 1 - ( Pi / _BlockStageWidth), r );
			}
			...
			{
				const float _BlocksVertical = _BlockStageWidth / 4.0;
				...
				// Compute angles
				float xAngle = ( pos.x / _BlockStageWidth ) * TwoPi;   // Angle due to X-Axis ( Horizontal wrap on sphere )
			...
			{ 
				float xAngle = ( pos.x / _BlockStageWidth ) * Pi * 2.0;  // Angle due to X-Axis ( Horizontal wrap on can )
			...
			v2f vert( appdata v ) 
			{
				...
				// Transform from grid to sphere
				pos.xyz = SphericalWarp( pos.xyz );
				// pos.xyz = CylindricalWarp( pos.xyz );

				// 
				pos.xyz *= _ApparentScale;
